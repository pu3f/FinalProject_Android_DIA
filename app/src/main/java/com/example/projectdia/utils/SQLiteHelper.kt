...
class SQLiteHelper(context: Context) :
    ...
    fun insertJob(job: JobPosition): Long{
        ...
        contentValue.put(ID, job.id)
        contentValue.put(JOB_NAME, job.jobName)
        contentValue.put(IMAGE, job.image)
        val success = db.insert(TBL_JOB, null, contentValue)
        db.close()
        return success
    }
